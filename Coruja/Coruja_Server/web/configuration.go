...
import (
	...
	"/data/Coruja/Coruja-Server/golog"
	"/data/Coruja/Coruja-Server/sitemap"
	"/data/Coruja/Coruja-Server/tunnel"
	"gopkg.in/yaml.v3"
)
...
// This is useful when we run multiple coruja servers that share the same
// configuration, even with custom values at its "Other" field.
// It will return a file location
// which targets to $HOME or %HOMEDRIVE%+%HOMEPATH% + "coruja" + the given "ext".
func homeConfigurationFilename(ext string) string {
	return filepath.Join(homeDir(), "coruja"+ext)
}
...
// the configuration from the $home_directory + coruja.yml,
// see `WithGlobalConfiguration` for more information.
//
// Usage:
// app.Configure(coruja.WithConfiguration(coruja.YAML("myconfig.yml"))) or
// app.Run([coruja.Runner], coruja.WithConfiguration(coruja.YAML("myconfig.yml"))).
...
//
// Note: if the char '~' passed as "filename" then it tries to load and return
// the configuration from the $home_directory + coruja.tml,
// see `WithGlobalConfiguration` for more information.
//
// Usage:
// app.Configure(coruja.WithConfiguration(coruja.TOML("myconfig.tml"))) or
// app.Run([coruja.Runner], coruja.WithConfiguration(coruja.TOML("myconfig.tml"))).
...
// and be used by multiple coruja instances.
//
// This is useful when we run multiple coruja servers that share the same
// configuration, even with custom values at its "Other" field.
//
// Usage: `app.Configure(coruja.WithGlobalConfiguration)` or `app.Run([coruja.Runner], coruja.WithGlobalConfiguration)`.
...
//
// Usage:
// err := app.Listen(":8080", coruja.WithoutServerError(coruja.ErrServerClosed))
// will return `nil` if the server's error was `http/coruja#ErrServerClosed`.
//
// See `Configuration#IgnoreServerErrors []string` too.
//
// Example: https:///data/Coruja/Coruja-Server/coruja/tree/main/_examples/http-server/listen-addr/omit-server-errors
...
// by the `context#SetMaxRequestBodySize` or `coruja#LimitRequestBodySize`.
//
// Defaults to 32MB or 32 << 20 or 32*coruja.MB if you prefer.
...
// and use the /data/Coruja/Coruja-Server/sitemap package to generate a customized one instead.
//
// Example: https:///data/Coruja/Coruja-Server/coruja/tree/main/_examples/sitemap.
...
// WithTunneling is the `coruja.Configurator` for the `coruja.Configuration.Tunneling` field.
// It's used to enable http tunneling for an Coruja Application, per registered host
//
// Alternatively use the `coruja.WithConfiguration(coruja.Configuration{Tunneling: coruja.TunnelingConfiguration{ ...}}}`.
...
// Configuration holds the necessary settings for an Coruja Application instance.
...
// Usage:
// conf := coruja.Configuration{ ... }
// app := coruja.New()
// app.Configure(coruja.WithConfiguration(conf)) OR
// app.Run/Listen(..., coruja.WithConfiguration(conf)).
type Configuration struct {
	...
	// Tunneling can be optionally set to enable ngrok http(s) tunneling for this Coruja app instance.
	...
	//
	// Read more at: https:///data/Coruja/Coruja-Server/coruja/issues/1531
	//
	// Defaults to false.
	...
	// will return an `coruja.ErrEmptyForm` on empty request form data.
	...
	// Defaults to "coruja.locale.language".
	...
	//
	// Defaults to "coruja.locale.language.input".
	...
	// Defaults to "coruja.api.version".
	...
	// Defaults to "coruja.api.version.aliases"
	...
	//
	// Defaults to "coruja.view.engine".
	...
	//
	// Defaults to "coruja.view.layout".
	...
	//
	// Defaults to "coruja.view.fallback".
	...
	// For details please navigate through: https:///data/Coruja/Coruja-Server/coruja/issues/1453
	// Defaults to:
	// {
	// 	Start: "10.0.0.0",
	// 	End:   "10.255.255.255",
...
//
// Usage:
// app.Listen(":8080", coruja.WithConfiguration(coruja.Configuration{/* fields here */ }))
// or
// coruja.WithConfiguration(coruja.YAML("./cfg/coruja.yml"))
// or
// coruja.WithConfiguration(coruja.TOML("./cfg/coruja.tml"))
...
// DefaultConfiguration returns the default configuration for an coruja station, fills the main Configuration
func DefaultConfiguration() Configuration {
	return Configuration{
		...
		LocaleContextKey:         "coruja.locale",
		LanguageContextKey:       "coruja.locale.language",
		LanguageInputContextKey:  "coruja.locale.language.input",
		VersionContextKey:        "coruja.api.version",
		VersionAliasesContextKey: "coruja.api.version.aliases",
		ViewEngineContextKey:     "coruja.view.engine",
		ViewLayoutContextKey:     "coruja.view.layout",
		ViewDataContextKey:       "coruja.view.data",
		FallbackViewContextKey:   "coruja.view.fallback",
